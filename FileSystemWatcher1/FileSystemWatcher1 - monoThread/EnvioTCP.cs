...
namespace FileSystemWatcher1
{
    class EnvioTCP
    {
        ...
        public EnvioTCP(String strIP, int iPort, Logeador oLog)
        {
            this.IPDest = strIP;
            this.PortDest = iPort;
            this.Timeout = 4000;
            ...
            this.oLog = oLog;
            oLog.Escribe(TipoMsgLog.Debug, "Instancia EnvioTCP");
        }
        ...
        public int Conecta()
        {
            ...
            oLog.Escribe(TipoMsgLog.Debug, "**Conectado");
        ...
        public int Cierra()
        {
            stream.Close();
            client.Close();
            oLog.Escribe(TipoMsgLog.Debug, "**Cerrado");
            return 0;
        }
        ...
        public uint Comunica(string sInput, ref string sOutput)
        {
            ...
            try
            {
                ...
                if (stream.CanRead)
                {
                    oLog.Escribe(TipoMsgLog.Debug, string.Format("CanRead: {0} buffer:{1}", stream.CanRead, bResp.Length));
                    do
                    {
                        iLeidos = stream.Read(bResp, 0, 4);
                        iTamanoMsg = MakeDWord(bResp);
                        iFaltantes = iTamanoMsg;
                        oLog.Escribe(TipoMsgLog.Debug, string.Format("iLeidos: {0} {1}", iLeidos, iTamanoMsg));
                        while (iFaltantes > 0)
                        {
